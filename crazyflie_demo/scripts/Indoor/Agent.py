...
class Agent:
    ...
    def Dynam_Search_in_maze(self, matrix):
        ...
        if close_wall:
            ...
            if np.linalg.norm(np.subtract(self.current_pos[0], self.next_pos[0])) > self.res and np.linalg.norm(
                    vec) > 0:
        ...
        rospy.logdebug("vec: {}".format(vec))
        rospy.logdebug("before vec size: {}".format(np.linalg.norm(vec)))
        if np.linalg.norm(vec) > self.stepSizeLimit:
            try:

                temp = np.divide(vec, np.linalg.norm(vec))
                rospy.logdebug("temp: {}".format(temp))
                vec = np.multiply(temp, self.stepSizeLimit)
            except Exception as err:
                print (err)
                rospy.logdebug("catched vec size: {}".format(np.linalg.norm(vec)))
        rospy.logdebug("after vec size: {}".format(np.linalg.norm(vec)))
